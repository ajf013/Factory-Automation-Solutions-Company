  ...
  return (
    <main className="relative min-h-screen">
      <div className="relative z-10 px-4 md:px-12 max-w-7xl mx-auto py-20 pt-32">
        ...
        >
          <h1 className="text-6xl md:text-8xl font-black text-white mb-6 tracking-tight uppercase leading-none text-shadow-xl">
            OUR <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-fasco-primary to-fasco-secondary text-shadow-blue text-glow">SOLUTIONS</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-32 lg:pr-[30%]">
          {solutions.map((sol, i) => (
            <motion.div
              ...
              className="p-10 rounded-3xl glass-strong border border-white/20 bg-white/70 hover:bg-white transition-all flex flex-col items-start group shadow-xl"
            >
              ...
              <h2 className="text-2xl font-black text-slate-900 mb-4 leading-tight uppercase tracking-tight group-hover:text-fasco-primary transition-colors">{sol.title}</h2>
              <p className="text-slate-700 text-sm leading-relaxed mb-8 flex-grow font-bold">{sol.desc}</p>
              <Link 
                href={sol.link}
                className="inline-flex items-center gap-2 text-fasco-primary font-black text-sm group/link uppercase tracking-wider"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-32">
          ...
          <motion.div
            ...
            className="p-10 md:p-14 rounded-[40px] glass-strong border border-white/20 bg-white/80 relative overflow-hidden shadow-2xl"
          >
            <div className="absolute top-0 right-0 w-64 h-64 bg-fasco-primary/10 blur-[100px] pointer-events-none" />
            <h3 className="text-4xl font-black text-slate-900 mb-10 tracking-tight uppercase">Get an <span className="text-fasco-primary">Instant Quote</span></h3>
            <form className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Full Name</label>
                <input type="text" placeholder="Your Name" className="w-full bg-black/5 border border-black/5 rounded-2xl px-6 py-4 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:bg-white focus:ring-4 focus:ring-fasco-primary/5 transition-all" />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Email Address</label>
                <input type="email" placeholder="Email Address" className="w-full bg-black/5 border border-black/5 rounded-2xl px-6 py-4 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:bg-white focus:ring-4 focus:ring-fasco-primary/5 transition-all" />
              </div>
              <div className="space-y-3 md:col-span-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Product Interest</label>
                <input type="text" placeholder="e.g. Robotic Arm" className="w-full bg-black/5 border border-black/5 rounded-2xl px-6 py-4 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:bg-white focus:ring-4 focus:ring-fasco-primary/5 transition-all" />
              </div>
              <button className="md:col-span-2 py-5 bg-fasco-primary hover:bg-fasco-primary/90 text-white font-black rounded-2xl transition-all shadow-2xl shadow-fasco-primary/40 active:scale-95 uppercase tracking-widest text-sm mt-4">
          ...
          <div className="flex flex-col justify-center lg:pl-10">
            <h3 className="text-5xl font-black text-white mb-12 tracking-tighter text-shadow-xl">INDUSTRIES <br /><span className="text-fasco-primary uppercase">WE SERVE</span></h3>
            <div className="flex flex-wrap gap-4">
              {weServe.map((ind, i) => (
                <motion.div
                  ...
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.05 }}
                  className="px-8 py-4 rounded-2xl glass-strong text-sm text-slate-900 font-black hover:text-fasco-primary hover:border-fasco-primary/30 transition-all border border-white/20 bg-white/70 shadow-xl"
                >
                  {ind}
                </motion.div>
              ))}
